...
namespace osuCrypto
{
        ...
        u64 numThreads)
    {
        ...
        for (u64 t = 0; t < numThreads; ++t)
        {
                ...
                &remainingPK0s, &PK0Prom, nSndVals]()
            {
                ...
                PRNG prng(seed);
                EllipticCurve curve_t(params, prng.get<block>());

                auto& g = curve_t.getGenerator();
                u64 fieldElementSize_t = g.sizeBytes();

                EccPoint PK0(curve_t);
                EccBrick bg(g);
                ...
                {
                    pC.emplace_back(curve_t);

                    pC[u].fromBytes(&*pBufIdx);
                    pBufIdx += fieldElementSize_t;
                }

                auto iter = sendBuff.data() + mStart * fieldElementSize_t;
                ...
                std::vector<u8>buff(fieldElementSize_t);
                EccBrick bc(pC[0]);
        ...
        u64 numThreads)
    {
        ...
        for (u64 t = 0; t < numThreads; ++t)
        {
            ...
            {
                EllipticCurve curve_t(params, seed);
                EccPoint pPK0(curve_t), PK0a(curve_t), fetmp(curve_t);
                EccNumber alpha2(curve_t, alpha);

                std::vector<EccPoint> c;
                c.reserve(nSndVals);
                for (u64 i = 0; i < nSndVals; ++i)
                    c.emplace_back(curve_t, pC[i]);
